...
///////////////////////////////////////////////////////////////////////////////
	...
	public void storeInfoMotify(StoreDTO sDto) {
		...
		try {
			...
			if (orgDto.getName().equals(sDto.getName())) {
				System.out.println("update문 처리~");
				pps = con.prepareStatement(
						"UPDATE storeinfo SET owner = ?, opendate = ?, closedate = ?, phone = ?, address = ? WHERE name = ?");
				pps.setString(1, sDto.getOwner());
				pps.setString(2, sDto.getOpendate());
				pps.setString(3, sDto.getClosedate());
				pps.setInt(4, sDto.getPhone());
				pps.setString(5, sDto.getAddress());
				pps.setString(6, sDto.getName());
				pps.executeUpdate();
			}
		} catch (SQLException e) {
			e.printStackTrace();
