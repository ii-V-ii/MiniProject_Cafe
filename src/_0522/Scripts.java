...
interface MainMenu {
	String SALES = "1", STORE = "2", MENU = "3", CUSTOMER = "4", STAFF = "5";
}
...
public class Scripts {
	...
	public void mainMenu() {
		send("1. 판매 | 2.매장관리  |  3.메뉴관리  |  4.고객관리  |  5.직원관리  |  6.프로그램 종료");
		send("선택 >>");
		choose = receive();
...
// 1차==========================================================================
	...
	public MemberDTO pointPlus() {
		...
		case "y":
			...
			send("회원 가입시는 new 를 입력하세요");
			choose = receive();
			if (choose.equals("new")) {
				custenroll();
			} else {
				member.setPhone(Integer.parseInt(choose));
			}
			break;
		case "n":
...
// 고객관리>회원정보 내부 메뉴
	public void showMembers() {// 1.모두보기
		...
		send("=======회원정보 모두보기=======");
		send("회원번호\t이름\t전화번호\t성별\t생년월일\t포인트");
		for (int i = 0; i < showMembersList.length; i++) {
			if (!showMembersList[i].getMemberID().contentEquals("0")) {
				send(showMembersList[i].getMemberID() + " | " + showMembersList[i].getName() + " | "
						+ showMembersList[i].getPhone() + " | " + showMembersList[i].getSex() + " | "
						+ showMembersList[i].getBirth() + " | " + showMembersList[i].getPoint());
			}
		}
...
// 고객관리>회원정보>정보보기 내부 메뉴(1.수정)
	public void showMemberDetailModify() {
		...
		for (MemberDTO member : memberDetailModifyList) {
			send("" + member.getName() + "\t" + member.getPhone() + "\t" + member.getSex() + "\t" + member.getBirth()
					+ "\t" + member.getMemberID());
		}
...
// 고객정보 수정할내용//////////////////////////////////////////////////소미파트
	...
	public void searchMembers() {
		send("검색 할 직원의 이름을 입력하세요");
		choose = receive();

		MemberDTO[] memberList = posControl.searchMember(choose);
		for (MemberDTO member : memberList) {
			send(member.getMemberID() + "\t" + member.getName() + "\t" + member.getPhone() + "\t" + member.getBirth()
					+ "\t" + member.getSex() + "\t" + member.getPoint());
		}
		send("========회원정보 변경========");
		send("1.수정");
		send("2.삭제");
